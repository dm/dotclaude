...
Use `/commit` for all commits. It handles semantic format, file staging, and pre-commit validation.

**Key principles** (the `/commit` skill enforces these):
- Commit after EACH RED-GREEN-REFACTOR cycle — never batch multiple tasks
- Tests and implementation travel together in the same commit
- Refactoring commits are separate from feature/fix commits
- WIP/plan files update AFTER code is committed, not before
